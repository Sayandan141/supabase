...
// Extend SvelteKit's App namespace for type safety

declare global {
  namespace App {

    // Custom error handling (uncomment to use)
    // interface Error {

    // Define types for server-side locals
    interface Locals {

      // Supabase client with database schema
      supabase: SupabaseClient<Database>;

      // Function to safely retrieve session and user with JWT validation
    ...
    // Define types for page data passed to Svelte components
    interface PageData {
      // Session data for page rendering
      session: Session | null;
    }

    // Additional SvelteKit interfaces (uncomment to use)
    // interface PageState {}
    // interface Platform {}
  }
}
...
// Fetch color names from the public colors table
export const load: PageServerLoad = async ({ locals: { supabase } }) => {
  // Query up to 5 colors, ordered by name
  ...
  if (error) {
    console.warn('Failed to fetch colors:', error.message);
    return { colors: [] };
  }
